        ...
        if (saved) {
            try {
                ...
                if (parsed.listColumns && Array.isArray(parsed.listColumns)) {
                    // If we have saved columns, use them, but make sure we don't crash if keys changed.
                    // For now, trust the saved columns if they exist.
    ...
    function resetSettings() {
        // Reset to available defaults but only update UI.
        ...
        document.getElementById('settingHorizontalScroll').checked = defaultSettings.horizontalScroll;
        // Re-render columns based on default
        renderColumnSettingsList(defaultSettings.listColumns);
        checkSearchLimitWarning();
    }
    // Updated to accept optional override
    ...
    // Removed updateFloatingButtonVisibility
    ...
    function updateReportButtonDisabledState() {
        ...
        // チェックされた項目があれば、修正後の内容が空でも報告できるようにする
        // （ユーザーが意図的に空にすることもあるため）
        submitBtn.disabled = false;
    }
    function toggleReportField(checkbox) {
        // チェックボックスの親のform-report-rowを取得
    ...
    function closeReportPopup(event) {
        ...
        const popup = document.getElementById('reportPopup');
        popup.classList.add('hidden');
        // List Modeの場合、選択状態が継続しているならreportingSongを保持
        // Search Modeの場合はその場限りの報告なのでクリア
    ...
    function submitReport() {
        ...
        // Google Apps Script エンドポイントに送信
        const url = 'https://script.google.com/macros/s/AKfycbz1xZ1M2AWkHuDFkOy9Hb3sY3r7M7quHtnT4lVZqHV1SNikVds7K-gFDCURHRpR7T-4/exec?' +
    ...
    function fetchFreshSongData() {
        google.script.run
            .withSuccessHandler(data => onDataLoaded(data, false))
            .withFailureHandler(onError)
            .getSongData();
    }
    function switchTab(tabName) {
        // 報告モード中に全曲一覧に移動する場合は報告モードを終了
    ...
    function onDataLoaded(data, isCached) {
        ...
        // Smart Refresh / State Restoration Logic
        const lastTab = localStorage.getItem('lastActiveTab');
        if (lastTab === 'list') {
            switchTab('list');
            filterList(); // Re-apply filter if any
        } else {
            switchTab('search'); // Default to search
            performSearch(); // Re-apply search if any
            // Ensure search query focus if first load
